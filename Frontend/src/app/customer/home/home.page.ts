...
import { IonHeader, IonToolbar, IonTitle, IonContent, IonCard, IonCardHeader, IonCardTitle, IonCardContent, IonButton, IonList, IonListHeader, IonLabel, IonBadge, IonIcon, IonItem, IonModal, IonButtons, IonInput, IonActionSheet, IonRefresher, IonRefresherContent, IonSpinner, IonChip, IonToast, IonRadio, IonCheckbox } from '@ionic/angular/standalone';
...
import { star, starOutline, heart, heartOutline, trashOutline, createOutline, refreshOutline } from 'ionicons/icons';

addIcons({ star, starOutline, heart, heartOutline, trashOutline, createOutline, refreshOutline });
...
export class HomePage implements OnInit {
  ...
  refreshUserProfile(): void {
    ...
    this.apiService.getProfile().subscribe({
      next: (userData) => {
        ...
        console.log('✅ User profile refreshed, points:', userData.loyalty_points);
      },
  ...
  saveFavorite(): void {
    ...
    this.apiService.saveFavourite(this.favoriteName, this.selectedOrder.id).subscribe({
      next: (fav) => {
      ...
      error: (err) => {
        ...
        this.showToastMessage('Failed to save favorite. Please try again.', 'danger');
      }
    });
  }
  ...
  deleteFavorite(fav: Favourite): void {
    if (confirm(`Delete "${fav.name}" from favorites?`)) {
